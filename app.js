...
const TRANSLATIONS = {
  ...
  zh: {
    ...
    "partner.market": "市场", "partner.followers": "受众规模", "partner.channel": "渠道", "partner.type": "合作类型", "partner.activity": "活跃度", "partner.relevance": "相关度", "partner.audience": "可能受众", "partner.viewEvidence": "查看证据", "partner.review": "审核", "partner.openProfile": "打开主页", "partner.demoRecord": "公开资料演示", "partner.demoProfileNote": "姓名、账号、简介和主页链接均对应同一个公开账号。这是人工整理的资料快照，并非实时检索。", "partner.demoNoReference": "旧版合成测试案例", "partner.profileUnavailable": "主页不可用", "partner.audienceUnknown": "受众不明确", "partner.contactAvailable": "可联系",
...
Object.assign(TRANSLATIONS.en, {"case.title":"Eazo · Growth Learning case","case.open":"Open interview case ↗","case.export":"Export all approved candidates","case.note":"Carry public research into an experiment proposal. Export includes source snapshots and decisions, never private review comments.","case.import":"Import next-run case brief","case.exportEmpty":"Approve between 1 and 50 candidates before exporting.","case.exportChanged":"An approval or profile changed. Review the current records before exporting.","case.exported":"Approved research downloaded. Import the file in the Eazo case.","case.importError":"Could not import this case draft. Existing work was preserved. Finish any running discovery first.","case.imported":"Eazo case draft imported","case.synthetic":"Basis: synthetic teaching scenario + human judgment. Not validated growth evidence.","case.baseline":"Basis: baseline public research. Experiment not run.","case.manual":"Only the brief is prefilled. Attached priorities need manual translation into criteria; they are not applied by the backend. Generate a new plan and review every query.","case.edited":"The brief has been edited since import."});
Object.assign(TRANSLATIONS.zh, {"case.title":"Eazo · 增长学习案例","case.open":"打开面试案例 ↗","case.export":"导出全部已通过候选","case.note":"将公开研究带入实验提案。导出包含来源快照和判断，不包含私人人工审核备注。","case.import":"导入案例下一轮简报","case.exportEmpty":"请先审核通过 1 至 50 位候选人再导出。","case.exportChanged":"审核或主页信息已改变，请检查当前记录后再导出。","case.exported":"已下载通过审核的研究，请在 Eazo 案例中导入。","case.importError":"无法导入此案例草稿，现有工作已保留。请先等待正在运行的发现完成。","case.imported":"已导入 Eazo 案例草稿","case.synthetic":"依据：模拟教学场景 + 人工判断，不是已验证的增长证据。","case.baseline":"依据：基础公开研究，实验尚未执行。","case.manual":"仅预填简报。所附优先级需人工转为条件，不会被后端自动应用。请重新生成计划并审核每条查询。","case.edited":"简报在导入后已被编辑。"});
...
const offlineId = (prefix) => `${prefix}_${crypto.randomUUID?.().replaceAll("-", "").slice(0, 12) || Math.random().toString(16).slice(2, 14)}`;
const offlineNow = () => new Date().toISOString().replace(/\.\d{3}Z$/, "Z");
...
const OFFLINE_RAW = [
  { id: "demo_creator_001", name: "Jesse Showalter", handle: "@imjesseshow", platform: "instagram", url: "https://www.instagram.com/imjesseshow/", followers: 56000, market: "United States", type: "influencer", priority: "P2", activity: "high", relevance: "high", audience: ["web designers and no-code builders", "people building portfolios or personal websites"], bio: "Designer and educator sharing portfolio, web design, and AI workflow experiments.", samples: ["Portfolio redesign walkthrough for independent designers.", "Testing AI website tools in a real client workflow."] },
  { id: "demo_creator_004", name: "Charli Marie", handle: "@charliprangley", platform: "youtube", url: "https://www.youtube.com/@charlimarieTV", followers: 220000, market: "Canada", type: "kol", priority: "P2", activity: "moderate", relevance: "high", audience: ["freelance designers and independent creatives", "web designers and no-code builders"], bio: "Design leader and YouTube educator covering web design, product design, and creator workflows.", samples: ["How designers can present portfolio work clearly.", "No-code and website workflow lessons for small teams."] },
  { id: "demo_creator_008", name: "Mizko", handle: "@mizko", platform: "x", url: "https://x.com/mizko", followers: 91000, market: "Australia", type: "influencer", priority: "P3", activity: "moderate", relevance: "moderate", audience: ["web designers and no-code builders"], bio: "UX and product design educator; some content overlaps with portfolio and website workflows.", samples: ["Design systems thread for product teams.", "Landing page teardown and conversion notes."] },
  { id: "demo_creator_011", name: "Dann Petty", handle: "@dannpetty", platform: "web", url: "https://www.dannpetty.com/", followers: 12000, market: "United States", type: "influencer", priority: "Needs Review", activity: "unknown", relevance: "moderate", audience: ["freelance designers and independent creatives"], bio: "Independent designer publishing about freelancing, portfolios, and creative business.", samples: ["Freelance positioning notes for designers.", "Portfolio critique themes gathered from community reviews."] },
];

function offlineSignal(value, reason, evidence = []) { return { value, reason, evidence }; }
function offlineCreator(raw, timestamp) {
  const observed = {
    record_id: raw.id, display_name: raw.name, profile_url: raw.url, normalized_profile_url: raw.url.toLowerCase(),
    platform: raw.platform, follower_count: raw.followers, bio_text: raw.bio,
    content_samples: raw.samples.map((text, index) => ({ text, published_at: `2026-09-${String(8 - index).padStart(2, "0")}T08:00:00Z` })),
    discovery_mode: "controlled_demo", source_connector: "curated_public_fixture_v1", retrieved_at: timestamp,
    run_id: "offline_demo", query_id: "offline_query", query_text: "portfolio and AI website creators", search_angle: "core_topic",
  };
  const derived = {
    record_id: raw.id,
    activity: offlineSignal(raw.activity, raw.activity === "high" ? "Multiple recent samples show strong publishing momentum." : "Available samples show usable publishing evidence.", [`${raw.samples.length} public content samples were observed.`]),
    content_relevance: offlineSignal(raw.relevance, raw.relevance === "high" ? "Several campaign themes are directly observable." : "Some campaign themes are observable.", ["Portfolio, web-design, no-code, or AI-tool language is present."]),
    audience_size: offlineSignal(raw.followers >= 100000 ? "large" : raw.followers >= 10000 ? "medium" : "small", `Observed follower count is ${raw.followers.toLocaleString("en-US")}; audience size is context, not a quality verdict.`),
    market: offlineSignal(["United States", "Canada"].includes(raw.market) ? "target" : "outside", `Public text points to ${raw.market}.`, [`Observed market term: ${raw.market}.`]),
    actionability: offlineSignal("available", "A public profile path is visible.", ["Public profile URL is available."]),
    record_quality: offlineSignal("usable", "The record has a valid profile identity and usable public evidence."),
    extracted_at: timestamp,
  };
  const priorityReasons = raw.priority === "Needs Review"
    ? ["Available public evidence needs a human decision.", "A public profile path improves actionability.", "Audience fit should be checked before outreach."]
    : ["Relevant public evidence supports outreach consideration.", "A public profile path improves actionability.", "Audience size is context, not a quality verdict."];
  return {
    record_id: raw.id,
    summary: {
      record_id: raw.id, display_name: raw.name, handle: raw.handle, platform: raw.platform, channel: raw.platform,
      partner_type: raw.type, partner_type_confidence: "demo", profile_url: raw.url, follower_count: raw.followers,
      bio_text: raw.bio, market: raw.market, market_fit: ["United States", "Canada"].includes(raw.market) ? "target" : "outside",
      activity: raw.activity, relevance: raw.relevance, likely_audience: raw.audience, audience_confidence: raw.relevance === "high" ? "high" : "moderate",
      priority: raw.priority, priority_reasons: priorityReasons, review_status: "unreviewed", reviewed_at: null, created_at: timestamp,
      discovery_mode: "controlled_demo", source_connector: "curated_public_fixture_v1", run_id: "offline_demo", query_id: "offline_query",
    },
    detail: {
      record_id: raw.id, observed_facts: observed, derived_signals: derived,
      ai_audience_inference: { record_id: raw.id, likely_audience: raw.audience, evidence: ["Audience is inferred from observed public profile text and remains a hypothesis."], confidence: raw.relevance === "high" ? "high" : "moderate", provider: "offline_demo", model: "static-demo", prompt_version: "audience-only-v1", inferred_at: timestamp },
      priority_decision: { priority: raw.priority, reasons: priorityReasons, signal_evidence: { ...derived }, decided_at: timestamp },
      human_decision: null,
      partner_type: { partner_type: raw.type, confidence: "demo", basis: "Offline demo classification for presentation." },
      priority_summary: { priority: raw.priority, headline: raw.priority, reasons: priorityReasons },
      ...
      ai_audience_summary: { likely_audience: raw.audience, confidence: raw.relevance === "high" ? "high" : "moderate", evidence: ["Audience is inferred from public profile text and content samples."], provider: "offline_demo", model: "static-demo" },
      content_samples_preview: 2,
    },
  };
}
function offlineWorkspace() {
  const creators = OFFLINE_DEMO.creators.map((record) => record.summary);
  ...
  creators.forEach((item) => { priorityCounts[item.priority] += 1; channelCounts[item.channel] += 1; partnerTypeCounts[item.partner_type] += 1; });
  const runRows = OFFLINE_DEMO.runs.map((run) => run.row);
  const latest = runRows.at(-1) || null;
  return {
    overview: { total_creators: creators.length, priority_counts: priorityCounts, review_count: creators.filter((item) => item.review_status !== "unreviewed").length, latest_run: latest },
    latest_discovery: latest ? { retrieved: latest.retrieved, duplicates: latest.duplicates, new_partners: latest.new_creators, new_partner_yield: latest.new_creator_yield, mode: latest.discovery_mode, status: latest.status, completed_at: latest.completed_at, run_id: latest.run_id } : null,
    partner_pool: { total: creators.length, priority_counts: priorityCounts, reviewed: creators.filter((item) => item.review_status !== "unreviewed").length, unreviewed: creators.filter((item) => item.review_status === "unreviewed").length, channel_counts: channelCounts, partner_type_counts: partnerTypeCounts },
    runs: runRows, query_history: OFFLINE_DEMO.runs.flatMap((run) => run.query_history).reverse(), creators,
    icp_workspace: { current: null, hypotheses: [] },
    workspace: { database_name: "offline-growth-os-demo", providers: { controlled: { configured: true, label: "Curated public snapshots" }, instagram: { configured: false, label: "Apify Instagram" }, x: { configured: false, label: "X API recent search" } }, channels: offlineChannels() },
  };
}
function offlineWorkflow(originalBrief) {
  ...
  const campaignId = offlineId("campaign");
  const searchPlanId = offlineId("searchplan");
  const themes = /ai|website/i.test(brief) ? ["AI website tools", "web design"] : ["growth", "portfolio building"];
  const markets = /uk|ireland/i.test(brief) ? ["United Kingdom", "Ireland"] : ["United States", "Canada"];
  const queries = [
    ["instagram", `${themes[0]} creator`, "Finds Instagram accounts centered on the campaign theme.", "core_topic"],
    ["instagram", `${themes[1]} workflow`, "Looks for visual process and tutorial content.", "creator_workflow"],
    ["x", `building with ${themes[0]}`, "Finds people discussing hands-on work on X.", "core_topic"],
    ["youtube", `${themes[0]} tutorial for designers`, "Finds channels that teach the topic at explanatory length.", "core_topic"],
    ["web", `${themes[1]} blog for freelancers`, "Finds independent blogs covering the theme.", "professional_identity"],
  ].map(([platform, query_text, rationale, search_angle], index) => ({ query_id: `${searchPlanId}_q${String(index + 1).padStart(3, "0")}`, campaign_id: campaignId, platform, query_text, rationale, search_angle }));
  const workflow = {
    workflow_id: offlineId("workflow"), original_brief: brief,
    campaign_parse: { brief: { campaign_id: campaignId, original_brief: brief }, definition: { campaign_id: campaignId, goal: "Discover partners for potential collaborations", target_markets: markets, content_themes: themes, target_audience: ["designers", "freelancers"], exclusions: [] }, status: "complete", missing_required_fields: [], clarification_questions: [], provenance: { provider: "offline_demo", model: "static-demo", prompt_version: "campaign-brief-v1", parsed_at: offlineNow() }, error_code: null, error_message: null },
    draft_search_plan: { search_plan_id: searchPlanId, campaign_id: campaignId, status: "draft", queries, provenance: { provider: "offline_demo", model: "static-demo", prompt_version: "draft-search-plan-v1", generated_at: offlineNow() } },
  };
  OFFLINE_DEMO.workflows.set(workflow.workflow_id, workflow);
  return workflow;
}
function offlineRun(payload) {
  const workflow = OFFLINE_DEMO.workflows.get(payload.workflow_id);
  if (!workflow) throw Object.assign(new Error("Workflow not found."), { code: "workflow_not_found" });
  const actions = payload.actions || [];
  if (actions.length !== workflow.draft_search_plan.queries.length || actions.some((action) => !["approved", "edited", "rejected"].includes(action.decision))) {
    throw Object.assign(new Error("Review every query before execution."), { code: "query_review_invalid" });
  }
  const completed = offlineNow();
  if (!OFFLINE_DEMO.creators.length) OFFLINE_DEMO.creators = OFFLINE_RAW.map((raw) => offlineCreator(raw, completed));
  const query_history = workflow.draft_search_plan.queries.map((query, index) => ({
    run_id: "offline_demo", campaign_id: query.campaign_id, approved_search_plan_id: "offline_plan",
    query_id: query.query_id, platform: query.platform, source_connector: "curated_public_fixture_v1",
    query_text: query.query_text, search_angle: query.search_angle, execution_status: index < 4 ? "SUCCESS_WITH_RESULTS" : "SUCCESS_ZERO_RESULTS",
    retrieved: index < 4 ? 2 : 0, duplicates: index < 4 ? (index % 2) : 0, new_creators: index < 4 ? 1 : 0,
    new_creator_yield: index < 4 ? .5 : 0, error_code: null, started_at: completed, completed_at: completed,
  }));
  const run_summary = { run_id: "offline_demo", mode: "controlled", run_status: "COMPLETED", run_error_code: null, retrieved: 8, duplicates: 4, new_creators: OFFLINE_DEMO.creators.length, new_creator_yield: .5, signals: OFFLINE_DEMO.creators.length, audience_inferences: OFFLINE_DEMO.creators.length, priority_counts: { P1: 0, P2: 2, P3: 1, "Needs Review": 1 }, query_history };
  OFFLINE_DEMO.runs = [{ row: { run_id: "offline_demo", campaign_id: workflow.campaign_parse.definition.campaign_id, approved_search_plan_id: "offline_plan", discovery_mode: "controlled", status: "COMPLETED", error_code: null, started_at: completed, completed_at: completed, retrieved: 8, duplicates: 4, new_creators: OFFLINE_DEMO.creators.length, new_creator_yield: .5 }, query_history }];
  return { review: {}, approved_search_plan: {}, run_summary, creator_records: OFFLINE_DEMO.creators };
}
async function offlineApi(path, options = {}) {
  const body = options.body ? JSON.parse(options.body) : {};
  if (path === "/api/bootstrap") return offlineWorkspace();
  if (path === "/api/search-plan") return offlineWorkflow(body.original_brief);
  if (path === "/api/run-discovery") return offlineRun(body);
  const creatorMatch = path.match(/^\/api\/creators\/([^/]+)(?:\/review)?$/);
  if (creatorMatch && path.endsWith("/review")) {
    const record = OFFLINE_DEMO.creators.find((item) => item.record_id === decodeURIComponent(creatorMatch[1]));
    if (!record) throw Object.assign(new Error("Partner record not found."), { code: "creator_not_found" });
    const decision = { review_id: offlineId("creatorreview"), record_id: record.record_id, status: body.status, structured_reason: body.structured_reason || null, comment: body.comment || null, reviewed_at: offlineNow() };
    record.summary.review_status = decision.status; record.summary.reviewed_at = decision.reviewed_at; record.summary.review_reason = decision.structured_reason; record.summary.review_comment = decision.comment; record.detail.human_decision = decision;
    return decision;
  }
  if (creatorMatch) {
    const record = OFFLINE_DEMO.creators.find((item) => item.record_id === decodeURIComponent(creatorMatch[1]));
  ...
  throw Object.assign(new Error("Offline demo does not support this action."), { code: "default" });
}
...
async function api(path, options = {}) {
  if (OFFLINE_DEMO.enabled) return offlineApi(path, options);
  try {
    ...
    if (path === "/api/bootstrap") {
      OFFLINE_DEMO.enabled = true;
      showToast("Using offline presentation demo.");
...
async function generatePlan() {
  ...
  finally { state.planning = false; button.disabled = false; button.innerHTML = `<span>${t("brief.generate")}</span><span aria-hidden="true">→</span>`; }
}
...
function isCuratedPublicDemoRecord(record) {
  return String(record?.source_connector || "").toLowerCase().includes("curated_public_fixture");
}

function profileAction(record, detail = false) {
  if (isControlledDemoRecord(record)) {
    if (!isCuratedPublicDemoRecord(record) || !record?.profile_url) return `<span class="demo-profile-note" role="note" title="${escapeHtml(t("partner.demoNoReference"))}">◇ ${t("partner.demoNoReference")}</span>`;
    if (detail) return `<div class="demo-profile-reference" role="note"><div class="demo-profile-note detail"><strong>${escapeHtml(t("partner.demoRecord"))}</strong><span>${escapeHtml(t("partner.demoProfileNote"))}</span></div><a class="button mini profile-link" href="${escapeHtml(record.profile_url)}" target="_blank" rel="noopener noreferrer">${t("partner.openProfile")} ↗</a></div>`;
    return `<a class="button mini profile-link curated-demo-profile" href="${escapeHtml(record.profile_url)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(t("partner.demoProfileNote"))}">${t("partner.openProfile")} ↗</a>`;
  }
...
/* Local case handoff: explicit JSON exchange; no new backend state or execution. */
function caseSafeURL(value) {
  try { const u = new URL(value); return ['https:', 'http:'].includes(u.protocol) && !u.username && !u.password ? u.href : null; } catch { return null; }
}
function caseCandidate(detail) {
  const raw = detail.observed_facts || {}, human = detail.human_decision;
  if (human?.status !== 'approve' || !human.reviewed_at || !caseSafeURL(raw.profile_url)) throw new Error(t('case.exportChanged'));
  return {
    record_id: detail.record_id, display_name: raw.display_name || raw.profile_url,
    profile_url: caseSafeURL(raw.profile_url), platform: raw.platform,
    review: { decision: 'approve', reviewed_at: human.reviewed_at },
    provenance: { discovery_mode: raw.discovery_mode, source_connector: raw.source_connector, retrieved_at: raw.retrieved_at, run_id: raw.run_id, query_id: raw.query_id },
    evidence: [{ url: caseSafeURL(raw.profile_url), title: raw.display_name || raw.profile_url,
      observed: raw.bio_text || 'Stored profile URL; no biography observed.',
      checked_at: null, published_at: null }]
  };
}
function validateCaseBrief(value) {
  if (value?.schema !== 'growth-os.case-brief/v1' || value.case_id !== 'eazo-growth-learning-os'
    || value.status !== 'draft_not_permission_to_retrieve' || value.requires_human_query_review !== true
    || typeof value.original_brief !== 'string' || !value.original_brief.trim() || value.original_brief.length > 10000
    || ![1, 2].includes(value.criteria_version) || !['baseline_research', 'synthetic_scenario_and_human_judgment'].includes(value.basis)
    || !Array.isArray(value.research_priorities) || value.research_priorities.length > 12
    || !value.research_priorities.every(item => typeof item === 'string' && item.length <= 500)) throw new Error(t('case.importError'));
  return { original_brief: value.original_brief, criteria_version: value.criteria_version, basis: value.basis, research_priorities: [...value.research_priorities] };
}
function caseDownload(value, name) {
...
async function exportCaseCandidates() {
  const button = $('#export-case'); button.disabled = true;
  try {
    const fresh = await api('/api/bootstrap');
    const approved = (fresh.creators || []).filter(c => c.review_status === 'approve');
    if (!approved.length || approved.length > 50) throw new Error(t('case.exportEmpty'));
    const candidates = [];
    for (const creator of approved) candidates.push(caseCandidate(await api('/api/creators/' + encodeURIComponent(creator.record_id))));
    caseDownload({ schema: 'growth-os.case-candidates/v1', exported_at: new Date().toISOString(), origin: 'growth_os', candidates }, 'growth-os-approved-candidates.json');
    showToast(t('case.exported'));
  } catch (error) { showToast(error.message || t('error.default')); }
  finally { button.disabled = false; }
}
function renderCaseBriefOrigin() {
  const node = $('#case-brief-origin'); if (!node) return;
  node.hidden = !state.caseBrief;
  if (!state.caseBrief) return;
  const modified = $('#campaign-brief').value !== state.caseBrief.original_brief;
  node.innerHTML = '<strong>' + escapeHtml(t('case.imported')) + ' · V' + state.caseBrief.criteria_version + '</strong><p>' +
    escapeHtml(t(state.caseBrief.basis === 'synthetic_scenario_and_human_judgment' ? 'case.synthetic' : 'case.baseline')) +
    '</p><ul>' + state.caseBrief.research_priorities.map(item => '<li>' + escapeHtml(item) + '</li>').join('') +
    '</ul><p>' + escapeHtml(t('case.manual')) + (modified ? ' ' + escapeHtml(t('case.edited')) : '') + '</p>';
}
function bindCaseHandoff() {
  $('#export-case').addEventListener('click', exportCaseCandidates);
  $('#import-case-brief').addEventListener('click', () => $('#case-brief-file').click());
  $('#case-brief-file').addEventListener('change', async event => {
    const file = event.target.files[0]; if (!file) return;
    try {
      if (state.running || state.planning || file.size > 128 * 1024) throw new Error(t('case.importError'));
      const brief = validateCaseBrief(JSON.parse(await file.text()));
      if (state.running || state.planning) throw new Error(t("case.importError"));
      state.caseBrief = brief; $('#campaign-brief').value = brief.original_brief;
      // A previous plan cannot remain executable under a newly displayed brief.
      state.workflow = null; state.queryDecisions.clear(); state.queryInputs.clear(); state.lastRunSummary = null;
      $('#plan-section').hidden = true; $('#run-result').hidden = true; $('#run-discovery').disabled = true; $('#confirm-live').checked = false;
      renderCaseBriefOrigin(); setPage('discover'); showToast(t('case.imported'));
    } catch { showToast(t('case.importError')); }
    finally { event.target.value = ''; }
  });
  $('#campaign-brief').addEventListener('input', renderCaseBriefOrigin);
}
